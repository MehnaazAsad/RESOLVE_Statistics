"""
{This script calculates cosmic variance from mocks}
"""
...
def measure_mock_mf(survey, path):
    """
    ...
    for num in range(num_mocks):
        ...
        max_smf, phi_smf, err_smf, bins_smf, counts_smf = \
            diff_smf(logmstar_arr, volume, 0, True)
        max_bmf, phi_bmf, err_bmf, bins_bmf, counts_bmf = \
            diff_bmf(logmbary_arr, volume, 0, False, True)

        phi_smf_arr.append(phi_smf)
        phi_bmf_arr.append(phi_bmf)
        max_smf_arr.append(max_smf)
        max_bmf_arr.append(max_bmf)
        err_smf_arr.append(err_smf)
        err_bmf_arr.append(err_bmf)
...
def plot_all_mf(max_mock, phi_mock, err_mock, max_data, phi_data, err_data, 
    counts_data):

    fig4 = plt.figure()
    for idx in range(len(phi_mock)):
        plt.errorbar(max_mock[idx],phi_mock[idx],yerr=err_mock[idx],
        markersize=4,capsize=5,capthick=0.5)
    for i in range(len(phi_data)):
        text = plt.text(max_data[i], 10**-1.07, counts_data[i],
        ha="center", va="center", color="k", size=7)
    plt.errorbar(max_data,phi_data,yerr=err_data,markersize=4,capsize=5,
...
def main(args):
    ...
    print("Measuring mass function for mock")   
    max_mock, phi_mock, err_mock  = measure_mock_mf(survey, path_to_mocks)
    ...
    print("Plotting mass functions for mock and data")
    plot_all_mf(max_mock, phi_mock, err_mock, max_data, phi_data, err_data, 
        counts_data)
