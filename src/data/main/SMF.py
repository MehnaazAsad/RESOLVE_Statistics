...
menStd     = np.sqrt(Phi_resolve)/(V_resolve*dM_resolve)
Phi_resolve = Phi_resolve/(V_resolve*dM_resolve)  

fig1 = plt.figure(figsize=(10,8))
plt.yscale('log')
plt.xlim(8.9,11.5)
plt.errorbar(Max_resolve,Phi_resolve,yerr=menStd,fmt="rs--",    
...
def make_animation(i,j,k,l,m):
    ...
    for ax in [ax1,ax2,ax3,ax4,ax5]:
        ax.clear()
        SMF_RESOLVE = ax.errorbar(Max_resolve,Phi_resolve,yerr=menStd,\
