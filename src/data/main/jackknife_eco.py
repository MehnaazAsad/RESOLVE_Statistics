"""
{This script carries out jackknife resampling for ECO survey}
"""
...
catl, volume, cvar, z_median = read_data(catl_file, 'eco')
...
# X = np.column_stack(jackknife_smf_arr)
# X -= X.mean(axis=0) 
# fact = (N/(N - 1))
# cov_mat = np.dot(X.T, X.conj()) / fact

cov_mat = np.matrix(np.cov(jackknife_smf_phi_arr.T, bias=True))*(N-1)

# cov_mat = np.cov(jackknife_smf_arr)
# cov_mat = cov_mat/(N/((N-1)**2)) # N = number of sub-grids
