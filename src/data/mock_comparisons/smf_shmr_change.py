...
# Experiment 5% above and below best fit SHMR to see if SMF changes by the 
# same amount
Mhalo_characteristic = np.array([eco_bf_params[0], 1.05*eco_bf_params[0], 
    0.95*eco_bf_params[0]])
Mstellar_characteristic = np.array([eco_bf_params[1], 1.05*eco_bf_params[1], 
    0.95*eco_bf_params[1]])
Mlow_slope = np.array([eco_bf_params[2], 1.05*eco_bf_params[2], 
    0.95*eco_bf_params[2]])
Mhigh_slope = np.array([eco_bf_params[3], 1.05*eco_bf_params[3], 
    0.95*eco_bf_params[3]])
Mstellar_scatter = np.array([eco_bf_params[4], 1.05*eco_bf_params[4], 
    0.95*eco_bf_params[4]])
...
# plt.legend(loc='best', prop={'size': 20})
# plt.ylabel(r'% change in SMF')

# # SHMR perecentage difference
# p_percent_diff = ((y_arr[1] - y_arr[0])/y_arr[0])*100
# n_percent_diff = ((y_arr[2][1:] - y_arr[0])/y_arr[0])*100
...
## SHMR perecentage difference
p_percent_diff = ((y_arr[1] - y_arr[0][1:])/y_arr[0][1:])*100
n_percent_diff = ((y_arr[2][2:] - y_arr[0][1:])/y_arr[0][1:])*100
ax4.plot(x_arr[0][1:], p_percent_diff, c='mediumorchid', label=r'$+5\% \Theta$', lw=5)
ax4.plot(x_arr[0][1:], np.zeros(len(p_percent_diff)), c='#53A48D', label=r'$best\ fit$', lw=5)
ax4.plot(x_arr[0][1:], n_percent_diff, c='cornflowerblue', label=r'$-5\% \Theta$', lw=5)
