...
def diff_smf(mstar_arr, volume, h1_bool):
    ...
    if survey == 'eco' or survey == 'resolvea':
        ...
        if survey == 'eco':
            bin_max = np.round(np.log10((10**11.6) / 2.041), 1)
...
def get_err_data(survey, path):
    ...
    cov_mat = np.cov(phi_arr_red, rowvar=False) # default norm is N-1
    err_red = np.sqrt(cov_mat.diagonal())
    cov_mat = np.cov(phi_arr_blue, rowvar=False) # default norm is N-1
    err_blue = np.sqrt(cov_mat.diagonal())
...
def measure_all_smf(table, volume, data_bool):
    ...
    if data_bool:
        ...
        mstar_col = 'stellar_mass'
        max_total, phi_total, err_total, bins_total, counts_total = \
            diff_smf(np.log10(table[mstar_col]), volume, True)
        max_red, phi_red, err_red, bins_red, counts_red = \
            diff_smf(np.log10(table[mstar_col].loc[table[colour_col] == 'R']
            ), volume, True)
        max_blue, phi_blue, err_blue, bins_blue, counts_blue = \
            diff_smf(np.log10(table[mstar_col].loc[table[colour_col] == 'B']
            ), volume, True)
...
if model == 'hybrid':
    ...
    Mh_qs_arr = np.linspace(11.0, 15.5, 30)
    muc_arr = np.linspace(0.0, 3.0, 30)
    mus_arr = np.linspace(0.0, 3.0, 30)
    
def init():
    
    # commas required so that legend will plot
    # https://stackoverflow.com/questions/11983024/matplotlib-legends-not-working
    line1, = ax1.plot([],[],color='#E61A27',ls="-",linewidth=2)
    line2, = ax2.plot([],[],color='#E61A27',ls="-",linewidth=2)     
    line3, = ax3.plot([],[],color='#E61A27',ls="-",linewidth=2)     
    line4, = ax4.plot([],[],color='#E61A27',ls="-",linewidth=2)
    line5, = ax1.plot([],[],color='#5696B9',ls="-",linewidth=2)
    line6, = ax2.plot([],[],color='#5696B9',ls="-",linewidth=2)     
    line7, = ax3.plot([],[],color='#5696B9',ls="-",linewidth=2)     
    line8, = ax4.plot([],[],color='#5696B9',ls="-",linewidth=2)
...
def make_animation(i,j,k,l,ax1_catalog,ax2_catalog,ax3_catalog,ax4_catalog):
    ...
    for ax in [ax1,ax2,ax3,ax4]:
        ...
        smf_data_red = ax.fill_between(red_data[0], 
            np.log10((10**red_data[1])/(10**red_data[2])), 
            np.log10((10**red_data[1])*(10**red_data[2])), color='#E61A27', 
            alpha=0.3)#, label=r'$\textrm{red}_{\textrm{d}}$')
        smf_data_blue = ax.fill_between(blue_data[0], 
            np.log10((10**blue_data[1])/(10**blue_data[2])), 
            np.log10((10**blue_data[1])*(10**blue_data[2])), color='#5696B9', 
            alpha=0.3)#, label=r'$\textrm{blue}_{\textrm{d}}$') 
        ax.set_ylim(-5,-1)     
        ax.minorticks_on()
        if ax == ax3 or ax == ax4:
    ...
    line3, = ax3.plot(red_model1[0], red_model1[1],
        color='#E61A27', ls="-", linewidth=2) 
    line1, = ax1.plot(red_model2[0], red_model2[1],
        color='#E61A27', ls="-", linewidth=2)
    line2, = ax2.plot(red_model3[0], red_model3[1],
        color='#E61A27', ls="-", linewidth=2)
    line4, = ax4.plot(red_model4[0], red_model4[1],
        color='#E61A27', ls="-", linewidth=2)
    line7, = ax3.plot(blue_model1[0], blue_model1[1],
        color='#5696B9', ls="-", linewidth=2)
    line5, = ax1.plot(blue_model2[0], blue_model2[1],
        color='#5696B9', ls="-", linewidth=2)
    line6, = ax2.plot(blue_model3[0], blue_model3[1],
        color='#5696B9', ls="-", linewidth=2)
    line8, = ax4.plot(blue_model4[0], blue_model4[1],
        color='#5696B9', ls="-", linewidth=2)

    if model == 'hybrid':
        ax3.legend([line1], [r'$M_{h}^{q}=%4.2f$' % Mhalo], loc='lower left',
            prop={'size': 15})
        ax1.legend([line2,smf_data_red,smf_data_blue],
            [r'$M_{*}^{q}=%4.2f$' % Mstar,
            r'$\textrm{red}_{\textrm{d}}$', r'$\textrm{blue}_{\textrm{d}}$'],
            loc='lower left',prop={'size': 15})
        ax2.legend([line3],[r'$\mu=%4.2f$' % mu], loc='lower left', 
            prop={'size': 15})
        ax4.legend([line4],[r'$\nu=%4.2f$' % nu], loc='lower left', 
            prop={'size': 15})
...
if model == 'hybrid':
    anim = animation.FuncAnimation(plt.gcf(), make_animation, Mh_q_arr, 
        ...
        ax2_catalog, ax3_catalog, ax4_catalog,), interval=500, blit=False, 
        repeat=True)
elif model == 'halo':
...
if model == 'hybrid':
    anim.save('{0}_smf_hybrid_test.gif'.format(survey),writer='imagemagick')
elif model == 'halo':
    anim.save('{0}_smf_halo_test.gif'.format(survey),writer='imagemagick')