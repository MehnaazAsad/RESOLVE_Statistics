...
def main(args):
    ...
    if survey == 'eco':
        ...
        catl_file = path_to_processed + "gal_group_eco_stellar_buffer_volh1_dr2_ps.hdf5"
    ...
    for col in col_idxs:
        ...
        gal_group_df = models_add_cengrpcz(gal_group_df, col, grpid_col='ps_groupid_{0}'.format(col), 
            galtype_col='ps_grp_censat_{0}'.format(col), cen_cz_col='cz')
        ...
        gal_group_df = gal_group_df.loc[\
            (gal_group_df['ps_cen_cz_{0}'.format(col)] >= cz_inner_mod) &
            (gal_group_df['ps_cen_cz_{0}'.format(col)] <= cz_outer)].reset_index(drop=True)
