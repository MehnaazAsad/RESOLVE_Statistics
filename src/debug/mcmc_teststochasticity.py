    ...
    try: 
        ...
        if quenching == 'hybrid':
            ...
            f_red_cen, f_red_sat = halo_quenching_model(theta, gals_df_mock, \
                'vishnu')
        assign_colour_label_mock(f_red_cen, f_red_sat, \
            gals_df_mock)
        # v_sim = 130**3
...
def assign_colour_label_mock(f_red_cen, f_red_sat, df, drop_fred=False):
    ...
    df.insert(df.shape[1], 'f_red', np.zeros(len(df)))
...
def get_err_data(survey, path):
    ...
    for box in box_id_arr:
        ...
        for num in range(num_mocks):
            ...
            assign_colour_label_mock(f_red_c, f_red_s, mock_pd)
    ...
    return err_colour, phi_corr_mat_inv_colour#, eigenvector_subset
