...
print('Populating halos using behroozi shmr params')
# Random subset of grey lines from 68th percentile param values smhm_run6
param_subset = pd.read_csv('data/processed/test.csv', sep=',')
param_subset = param_subset.drop(columns=['Unnamed: 0'])
param_subset = param_subset.loc[:,'mhalo_c':'scatter']

fig1= plt.figure(figsize=(10,10))
for idx in range(len(param_subset)):
    # Slicing row by row of param values
    param_set = param_subset.iloc[idx:idx+1,:].values[0]
    gals_df_ = populate_mock(param_set, model_init)
    gals_df_ = assign_cen_sat_flag(gals_df_)
    gals_df_ = gals_df_[['stellar_mass', 'C_S', 'halo_mvir', \
        'halo_mvir_host_halo', 'halo_macc','halo_hostid', 'halo_id']]
    print('Getting best fit model')
    maxis_bf_red, phi_bf_red, maxis_bf_blue, phi_bf_blue, cen_gals_red, \
        cen_halos_red, cen_gals_blue, cen_halos_blue = \
