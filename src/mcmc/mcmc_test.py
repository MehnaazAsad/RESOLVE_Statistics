...
def write_to_files(sampler):
    ...
    data = sampler.chain
    fname = path_to_proc + 'mcmc_{0}_raw_test.txt'.format(survey)
    ...
    fname = path_to_proc + 'mcmc_{0}_test.dat'.format(survey)
    ...
    fname = path_to_proc + '{0}_chi2_test.txt'.format(survey)
    with open(fname, 'w') as outfile:
        for value in blobs:
