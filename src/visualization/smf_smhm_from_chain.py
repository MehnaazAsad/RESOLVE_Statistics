...
chi2_df = pd.read_csv(path_to_proc + 'eco_chi2.txt',header=None,names=['chisquared'])
test_reshape = chi2_df.chisquared.values.reshape((500,250))
chi2 = np.ndarray.flatten(np.array(test_reshape),'F')

chain_fname = chain_fname = path_to_proc + 'mcmc_eco.dat'
...
plt.errorbar(max_eco,phi_eco,yerr=err_eco,color='k',fmt='--s',ecolor='k',markersize=4,capsize=5,capthick=0.5,label='ECO',zorder=5)
for idx in range(len(result[0][0])):
    plt.errorbar(result[0][0][idx],result[0][1][idx],yerr=result[0][2][idx],color='lightgray',fmt='-s',ecolor='lightgray',markersize=4,capsize=5,capthick=0.5,label='model',alpha=0.1,zorder=1)
for idx in range(len(result[0][0])):
    plt.errorbar(result[1][0][idx],result[1][1][idx],yerr=result[1][2][idx],color='lightgray',fmt='-s',ecolor='lightgray',markersize=4,capsize=5,capthick=0.5,label='model',alpha=0.1,zorder=0)

plt.errorbar(max_model_bf,phi_model_bf,yerr=err_tot_model_bf,color='r',fmt='--s',ecolor='r',markersize=4,capsize=5,capthick=0.5,label='best fit',zorder=10)
plt.yscale('log')
plt.ylim(10**-5,10**-1)
...
plt.savefig(path_to_figures + 'smf_emcee_eco.png')
